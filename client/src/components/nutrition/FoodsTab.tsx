...
export const FoodsTab = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === "library") {
      ...
      // Load from client's food log entries (from users collection)
      const entriesRef = collection(db, 'users', clientId, 'foods');
      const entriesQuery = query(entriesRef, orderBy('timestamp', 'desc'));
      
      const unsubscribe = onSnapshot(entriesQuery, (snapshot) => {
        const loadedEntries: FoodEntry[] = [];
        
        snapshot.forEach((doc) => {
          const data = doc.data();
          // Get entries from each date document
          if (data.entries && Array.isArray(data.entries)) {
            data.entries.forEach((entry: any, index: number) => {
              loadedEntries.push({
                ...
                date: entry.timestamp?.toDate ? entry.timestamp.toDate().toLocaleDateString() : doc.id
              });
            });
          }
        });
        
        setFoods(loadedEntries);
        setLoading(false);
      });

      return () => unsubscribe();
    }
  }, [user?.uid, selectedClientId, activeTab]);
        ...
        <div className="mt-6">
          ...
          <TabsContent value="database">
            {loading ? (
              <div className="text-center py-12">
                <Loader2 className="w-8 h-8 text-gray-400 mx-auto mb-4 animate-spin" />
                <p className="text-gray-500">Loading food database...</p>
              </div>
            ) : (
              <>
                <div className="grid gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <div>
                                <h3 className="font-medium text-gray-900">{food.name}</h3>
                                <p className="text-sm text-gray-500">
                                  Client logged • {food.servingSize} {food.servingUnit}
                                </p>
                              </div>
                            </div>
                ...
                {filteredFoods.length === 0 && !loading && (
                  <div className="text-center py-12">
                    <Apple className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <h3 className="text-lg font-medium text-gray-900 mb-2">
                      {searchTerm ? 'No foods found' : 'No client food logs'}
                    </h3>
                    <p className="text-gray-500">
                      {searchTerm 
                        ? 'Try adjusting your search terms'
                        : 'Client food logs will appear here when they start tracking'
                      }
                    </p>
                  </div>
                )}
